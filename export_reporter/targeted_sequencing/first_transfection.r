...
segmented_filtered <- read_tsv("GASR/export_reporter/targeted_sequencing/data/first_experiment_barcodes_upis.txt.gz")
...
write_tsv(new_barcode_to_struct, "GASR/export_reporter/targeted_sequencing/minimap_reannotation/data/first_transfection_barcode_architectures.tsv.gz")
new_barcode_to_struct <- read_tsv("GASR/export_reporter/targeted_sequencing/minimap_reannotation/data/first_transfection_barcode_architectures.tsv.gz")

# Analyse -----------------------------------------------------------------

usable_counts_per_sample <- segmented_filtered %>% 
  filter(plasmid_bc %in% barcode_architectures$plasmid_bc) %>%
  count(sample, fraction, replicate)

count_usable_upis <- segmented_filtered %>%
  # mutate(sample = paste0(experiment, "_", sample)) %>%
  # sample_frac(0.01) %>%
  filter(plasmid_bc %in% barcode_architectures$plasmid_bc) %>%
  count(sample, exp_bc, replicate, fraction, plasmid_bc) %>%
  group_by(sample) %>%
...
#   separate(sample, into = c("fraction", "replicate"), remove = F) %>%
#   ggplot(aes(x = -PC1, y = PC2, colour = fraction)) +
#   geom_point(size = 3) +
...
ratio_per_replicate <- count_usable_upis %>%
  ...
  select(fraction, replicate, cpm, bc_number) %>%
...
write_tsv(l_plus_ratio, "GASR/export_reporter/targeted_sequencing/minimap_reannotation/data/first_transfection_gene_ratios.tsv.gz")

cc <- scales::seq_gradient_pal("#B4B4B4", "#B9529F", "Lab")(seq(0,1,length.out=5))
...
group_means %>%
  reframe(model = pairwise.t.test(log2(group_mean_ratio), mv_class) %>% tidy()) %>% 
  unnest(model) %>%
  # filter(!((group1 == "12hr dox") & (group2 == "8hr dox"))) %>%
  mutate(padj = p.adjust(p.value, method = "BH")) ->
  pairwise_t_tests

write_tsv(pairwise_t_tests, file = "GASR/export_reporter/targeted_sequencing/minimap_reannotation/plots/first_transfections_group_means_multivalency_ttests.tsv")
...
ggsave("GASR/export_reporter/targeted_sequencing/minimap_reannotation/plots/first_transfection_combiplot.pdf", combi_plot,
       width = 5, height = 3.5)
...
ggsave("GASR/export_reporter/targeted_sequencing/minimap_reannotation/plots/first_transfection_main_figure_combiplot.pdf", big_combi,
       width = 9, height = 3.5)
